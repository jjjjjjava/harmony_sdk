...
// 用于 longjmp 返回值的线程局部变量
extern __thread volatile int longjmp_value;

// 全局回调指针，用于在 FFmpeg 执行过程中报告状态
...
/**
 * 重置 FFmpeg 全局状态
 * 在 ffmpeg_cleanup 之后调用，确保下次执行时状态干净
 */
static void reset_ffmpeg_state(void)
{
    // 重置关键标志
    ffmpeg_exited = 0;
...
long mid_pred(long i, long i1, long i2) {
...
/**
 * 标准 main 入口点
 * 包装 exe_ffmpeg_cmd 用于命令行执行
 */
int main(int argc, char **argv) {
    return exe_ffmpeg_cmd(argc, argv, NULL);
}
